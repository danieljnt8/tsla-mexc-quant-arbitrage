...
========================
Data alignment and feature engineering for the Quant Approach (Strategy A).

Key differences from pluang_task preprocessor
----------------------------------------------
1. Spread formula: log(mexc_close / tsla_close) instead of arithmetic percent.
   Log-ratio is symmetric: a +5% MEXC premium and −5% MEXC discount produce
   equal-magnitude z-scores, which is important for a mean-reversion signal.

2. Lookahead-free z-score via shift(1):
   The existing system uses:
       rolling(W).mean()  ← includes current bar t in the window
   The Quant approach uses:
       spread.shift(1)     ← shifts back one bar
       shifted.rolling(W)  ← at bar t, uses bars [t-W, t-1] only
   This ensures the z-score at time t is computed ONLY from past data and
   does NOT include the extreme spread value that triggered the signal.
   It is the correct, look-ahead-free implementation.

Spread interpretation
---------------------
  log_spread > 0  → MEXC is trading ABOVE TSLA (positive premium)
  log_spread < 0  → MEXC is trading BELOW TSLA (negative discount)
  log_spread ≈ 0  → prices in equilibrium

Z-score interpretation
----------------------
  z > +2.0  → spread is 2 standard deviations above its recent average
               → MEXC is unusually expensive → SHORT MEXC, LONG TSLA
  z < -2.0  → spread is 2 standard deviations below its recent average
               → MEXC is unusually cheap → LONG MEXC, SHORT TSLA

Warm-up period
--------------
  The first `window + 1` bars will have NaN z-scores:
    - 1 bar lost to shift(1)
    - `window` bars for the rolling window to accumulate enough history
  The engine and strategy skip NaN z-score bars automatically.
"""
